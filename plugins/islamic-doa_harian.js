let handler = async (m, { conn }) => {
    try {
        const doaList = getDoaHarianData();
        
      
        const randomDoa = pickRandom(doaList);

        
        let responseText = `*━━━ • Doa Harian • ━━━*\n\n`;
        responseText += `*${randomDoa.title}*\n\n`;
        responseText += `*Arab:*\n${randomDoa.arabic}\n\n`;
        responseText += `*Latin:*\n${randomDoa.latin}\n\n`;
...
function getDoaHarianData() {
    return [
        ...
        {
            "title": "Doa Keluar Rumah / Bepergian",
