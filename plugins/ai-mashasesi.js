...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    if (text.toLowerCase() === "on") {
        ...
        m.reply("[ ✓ ] Berhasil Membuat Sesi Chat Masha-AI");
    } else if (text.toLowerCase() === "off") {
        delete conn.betaai[m.sender];
        m.reply("[ ✓ ] Berhasil Menghapus Sesi Chat Masha-AI");
    } else {
        ...
        if (json.message) {
            await conn.sendMessage(m.chat, {
                ...
                contextInfo: {
                    externalAdReply: {
                        mediaType: 1,
                        title: "Masha AI - Jawaban dari Pertanyaanmu",
                        body: "Masha AI siap membantu 😄",
...
handler.command = ['mashaai'];
handler.tags = ['ai'];
handler.help = ['mashaai [on/off]'];
